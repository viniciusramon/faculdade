...
DELIMITER $$
CREATE FUNCTION puxaNomes(pEspecialidade VARCHAR(50)) RETURNS VARCHAR(200)
BEGIN
   DECLARE retorno VARCHAR(200);
   SELECT GROUP_CONCAT(nome) INTO retorno
     FROM medico
    WHERE  especialidade LIKE CONCAT(pEspecialidade,"%")
    GROUP BY especialidade;
   RETURN retorno;
 END;
$$

SELECT puxaNomes('Orto')
  FROM dual;

DELIMITER $$
CREATE FUNCTION qtdeDias(plucas INT) RETURNS INT
BEGIN
   DECLARE nomeVar INT;
	SELECT DATEDIFF(dataHora, CURDATE()) INTO nomeVar
	  FROM consulta
	 WHERE id = plucas;
   RETURN nomeVar;
END;
...
SELECT id, qtdeDias(id)
  FROM consulta;

DELIMITER $$
CREATE FUNCTION contaConsulta(vNomePac VARCHAR(50), vNomeMed VARCHAR(50)) RETURNS INT
BEGIN
   DECLARE retorno INT;
	SELECT COUNT(*) INTO retorno
	  FROM consulta
		   ...
		   JOIN paciente
			 ON paciente.rg = consulta.rg
	 WHERE paciente.nome = vNomePac
	   AND medico.nome   = vNomeMed;
   RETURN  retorno;
END;
$$
       
SELECT contaConsulta('Jarbeas Data', 'Lois Lane')
  FROM medico;


SELECT paciente.nome, medico.nome, contaConsulta(paciente.nome, medico.nome)
  FROM consulta
	   JOIN medico
		 ON medico.crm = consulta.crm
	   JOIN paciente
		 ON paciente.rg = consulta.rg
-- 1 - Contar quantas consultas um determinado médico fez num período de 
-- tempo (passa-se o médico, a data início e data fim e a função retorna 
-- quantas consultas aquele médico fez).

DELIMITER $$
CREATE FUNCTION contaConsultas(dataInicio DATETIME, 
                               dataFim    DATETIME,
                               crmMedico  INT) RETURNS INTEGER
BEGIN
   DECLARE retorno INTEGER;
	SELECT COUNT(*) INTO retorno
	  FROM consulta
	 WHERE dataHora BETWEEN dataInicio
						AND dataFim
	   AND consulta.crm = crmMedico;
   RETURN retorno;
END;
$$

SELECT contaConsultas('2021-08-21', '2021-08-29', 79)
  FROM dual;
 
 SELECT *
   FROM dual;

-- 2 - Contar quantos médicos atendem num determinado andar (passa-se o 
-- andar, a função retorna quantos médicos DISTINTOS atendem naquele andar)
...
END;
$$
SELECT contaMedicoAndar(2)
  FROM dual;
  
SELECT andar, contaMedicoAndar(andar)
  FROM ambulatorio;
-- 3 - Contar quantos pacientes moram em determinada cidade (passa-se a 
-- cidade, a função retorna quantos pacientes moram naquela cidade).
DELIMITER $$
...
-- DADO UM RANGE SALARIAL, MOSTRA UMA LISTA DE FUNCIONÁRIOS
-- QUE RECEBAM AQUELE SALÁRIO

DELIMITER $$
CREATE FUNCTION mostraFuncionario(inferior FLOAT,
                                  superior FLOAT) RETURNS VARCHAR(250)
BEGIN
...
-- MOSTRAR A LOTAÇÃO ATUAL DE CADA AMBULATÓRIO
SELECT ambulatorio.numeroA, ambulatorio.capacidade, COUNT(*)
  FROM ambulatorio
       JOIN medico
	...
	SELECT CONCAT(((COUNT(*) / ambulatorio.capacidade) * 100), "%")
	  ...
	  FROM ambulatorio
		   ...
		   JOIN (SELECT rg, ultimaConsulta(rg) dataUltima
			 ...
			 ON consulta.rg = ultima.rg
			AND consulta.dataHora = dataUltima
...
DELIMITER $$
	CREATE PROCEDURE insertMedico(v_CRM int, v_nome varchar(50), v_idade int, v_cidade varchar(50), v_especialidade varchar(50), v_numeroA int)
		BEGIN 
			insert into medico (CRM, nome, idade, cidade, especialidade, numeroA)
				VALUES (v_CRM, v_nome, v_idade, v_cidade, v_especialidade, v_numeroA);
        END;
	$$
    
    CALL insertMedico(1111, 'Barreto Johnson', 33, 'Americana/SP', 'Dentista', 2);
...
insert into nota(valorTotal)
	values (null),
			(null),
            (null);
            
	insert into produto(valorUnitario, nome)
		values (20, 'Arroz'),
				(8, 'Feijão'),
                (45, 'Carne'),
                (5, 'Tomate'),
     ...
     drop procedure insereProdutoNF;
     
	delimiter $$
		create procedure insereProdutoNF(v_idNota int, v_idProduto int)
			begin
            
            -- cria uma VAR para receber a soma dos valores dos produtos comprados
				declare pegarValorTotal float;
                
                -- insere os dados na tabela e produtoNF
					insert into produtoNF(idProduto, idNota)
						values (v_idProduto, v_idNota);
                        
				-- busco o valor total da NF
                select sum(valorUnitario) into pegarValorTotal
					from produtoNF
						join produto
							on produto.id = produtoNF.idProduto
					where idNota = v_idNota;
                    
				-- faço a alteração da NF
                update nota
					set valorTotal = pegarValorTotal
				where id = v_idNota;
            end;
    $$
    
    call insereProdutoNF(2,5);
    select * from nota;;
    select * from produtonf;
    
delimiter $$
	create trigger barreto after insert on produtoNF for each row
		begin
			call insereProdutoNF(new.idNota, new.idProduto);
        end;
$$

call insereProdutoNF(1,1);
select * from nota;

delimiter $$
	create procedure atualizaValorTotal(v_idNota int)
		begin
			declare pegarValorTotal float;
			-- buscar todos os itens da NF, somar tudo e dar o valor total
				select sum(valorUnitario) into pegarValorTotal
					from produtoNF
						join produto
							on produtoNF.idProduto = produto.id
					where idNota = v_idNota;
                    
			-- atualizar na tabela nota o valor total calculado acima
            update nota
				set valorTotal = pegarValorTotal
			where id = v_idNota;
        end;
...
delimiter $$
	create trigger calculaValorTotalNF after insert on produtoNF
		for each row
			begin
				call atualizaValorTotal(new.idNota);
			end;
$$

drop trigger barreto;

insert into produtoNF(idNota, idProduto)
	values (3,2);
